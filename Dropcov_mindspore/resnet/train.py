...
def train_net():
    ...
    # define opt
    lr1=np.full(30*5004,0.12)
    lr2=np.full(30*5004,0.012)
    lr3=np.full(30*5004,0.0012)
    lr4=np.full(30*5004,0.00012)
    lr=np.stack([lr1,lr2,lr3,lr4],axis=0).reshape(120*5004)
    #lr = ms.Tensor(init_lr(step_size=step_size))
    lr=ms.Tensor(lr)
    group_params = init_group_params(net)
    opt = nn.Momentum(group_params, lr, config.momentum,loss_scale=512)
