...
class ResNet(nn.Cell):
    ...
    def __init__(self,
        ...
        self.eca_layer = eca_layer(channel=128)
        self.cov=Covariance()
    ...
    def balance_rate(self, x, y):
        ...
        op=ops.ReduceSum()
        cor = op(self.cov(x), 2)
        lpn=ops.LpNorm(axis=1,p=2)
        cor_, y_ = lpn(cor), lpn(y)
        mul=ops.Mul()

        rate = mul(y, cor).sum(axis=1) / (y_ * cor_) 
        rmean=rate.mean()  
        #npd=math.log2(d)
        b_rate = 1 - (10/ops.log2(ts([d],mindspore.float32))) * (rmean) 
        b_rate = ops.clip_by_value(b_rate, 0, 1)   
        x,_= ops.dropout2d(x,float(b_rate.item())) 
        return x  
