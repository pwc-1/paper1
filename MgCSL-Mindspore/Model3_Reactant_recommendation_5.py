...
def GetAtomStructure(Product_Mol, Bond, X):   # 得到断裂键周围X深度的结构
	...
	mw.ReplaceAtom(int(0), Chem.Atom(33))  # 将列表中的原子转换成“*”
	Mol = mw.GetMol()
...
class Classify(nn.Cell):
	def __init__(self, in_features, out_features, bias=True):
		...
		self.linear2 = nn.Dense(out_features, 1)
		self.use_bias = bias
		if self.use_bias:
			self.bias = Parameter(Tensor(np.zeros(out_features).astype(np.float32)))

	def construct(self, input_features, Labels_Features_GCN):
		print('2122', self.in_features, self.out_features)
		print('2122', input_features.shape)
