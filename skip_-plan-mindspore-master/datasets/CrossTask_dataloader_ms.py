...
def get_A(task_steps, share="words", vocab_size=None):
    ...
    for task, steps in task_step_comps.items():
        K = len(steps)
        #a = ms.zeros((M, K), dtype=ms.float32)
...
class CrossTaskDataset:
    ...
    def __getitem__(self, idx):
        ...
        #     if idx == 0:
        #         tensors = [ Tensor(frames[i-1]),
        #                     Tensor(frames[i+1]),
        #                     Tensor(frames[i])]
        #     else:
        #         tensors = [ Tensor(frames[C[idx - 1][1]] - 1),
        #                     Tensor(frames[i]),
        #                     Tensor(frames[i+1])]
        #     stacked_tensor = ops.stack(tensors)
        #     tmp_X.append(stacked_tensor)
        for idx, (i, _) in enumerate(C):
            if idx == 0:
                tensors = [ (frames[i-1]),
                            (frames[i+1]),
                ...
                tensors = [ (frames[C[idx - 1][1]] - 1),
                            (frames[i]),
                            (frames[i+1])]
            stacked_tensor =np.stack(tensors)
            tmp_X.append(stacked_tensor)
        ...
        # tensors = [ 
        #             Tensor(frames[last_frame - 1]),
        #             Tensor(frames[last_frame]),
        #             Tensor(frames[last_frame + 1]),
        #             ]
        # stacked_tensor = ops.stack(tensors)
        tensors = [ 
        ...
        for idx, (i, _) in enumerate(C):
            if idx == 0:
                ...
                tensors = [ (frames[C[idx - 1][1]] - 1),
                            ...
                            (frames[i+1])]
            stacked_tensor = np.stack(tensors)
            tmp_X_ori.append(stacked_tensor)

        last_frame = min(C[-1][-1], frames.shape[0] - 2)
        
        # tensors = [ Tensor(frames[last_frame - 1]),
        #             Tensor(frames[last_frame]),
        #             Tensor(frames[last_frame + 1]),
        #             ]
        # stacked_tensor = ops.stack(tensors)
        tensors = [ (frames[last_frame - 1]),
                    (frames[last_frame]),
                    (frames[last_frame + 1]),
                    ]
        stacked_tensor = np.stack(tensors)
        ...
        X_ori = np.stack(tmp_X_ori)
       
     
        #X_ori = X_ori.astype(ms.float32)
        X_ori = X_ori.astype(np.float32)
        X = np.concatenate([X, X_ori[:, :, -128:]], -1)
