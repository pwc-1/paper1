...
class MLP(nn.Cell):
    def __init__(self, input_dim, output_dim, hidden_size=(1024, 512), activation='relu', discrim=False, dropout=-1):
        ...
        for i in range(len(dims)-1):
            self.layers.append(nn.Dense(dims[i], dims[i+1]))
        ...
        self.sigmoid = ops.Sigmoid() if discrim else None
    ...
    def construct(self, x):
        ...
        for i in range(len(self.layers)):
            ...
            if i != len(self.layers)-1:
                x = self.activation(x)
                if self.dropout != -1:
                    x = nn.Dropout(keep_prob=1-(min(0.1, self.dropout/3) if i == 1 else self.dropout))(x)
            elif self.sigmoid:
                x = self.sigmoid(x)
        return x

def tfm_mask(seg_per_video, temporal_multiplier=1):
    """
    Attention mask for padded sequence in the Transformer
    True: not allowed to attend to
    """
    B = len(seg_per_video)
    L = max(seg_per_video) * temporal_multiplier
    mask = ops.ones((B,L),dtype=ms.bool)
    for ind, l in enumerate(seg_per_video):
        mask[ind, :(l * temporal_multiplier)] = False
    return mask
...
def save_checkpoint(state, is_best, config, filename='checkpoint.ckpt'):
    checkpoint_path = os.path.join(config['output_dir'], config['model_name'], filename)
    model_path = os.path.join(config['output_dir'], config['model_name'], 'model_best.ckpt')
    ...
    if state['epoch'] % config.get('saving_checkpoints_iter', 5) == 0:
        ...
        iter_model_path = os.path.join(config['output_dir'],
                                       config['model_name'],
                                       'model_{}.ckpt'.format(state['epoch']))
