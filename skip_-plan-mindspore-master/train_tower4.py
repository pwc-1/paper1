...
test_loader = ds.GeneratorDataset(testset,   [ 'vid', 'task', 'X', 'C', 'W', 'T'], shuffle=False,
                                  num_parallel_workers=args.num_workers)
...
def main():
    ...
    if args.resume:
        ...
        checkpoint = ms.load_checkpoint(args.resume,net=model)
    ...
    # def dynamic_lr(lr, total_epoch):
    #     lrs = []
    #     for i in range(total_epoch):
    #         decay = 0.1 ** (sum(i >= np.array(args.lr_steps)))
    #         lrs.append(lr * decay)
    #     return lrs

   # decay_lr = dynamic_lr(lr=args.lr, total_epoch=args.epochs)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        loss, acc, success_rate, miou = validate(args, test_loader, model, criterion, epoch, tb_logger)
...
def train(args, train_loader, model, optimizer, epoch, criterion, tb_logger):
    ...
    def forward_fn(frames, lowlevel_labels):
        ...
        loss4 = criterion(output_reshaped, lowlevel_labels_reshaped.long())

        lowlevel_labels2 = ms.ops.cat([lowlevel_labels[:, 0:2], lowlevel_labels[:, 3:4]], axis=1)
        output_reshaped2 = reshape_op(output2, (-1, output2.shape[-1]))
        lowlevel_labels_reshaped2 = reshape_op(lowlevel_labels2, (-1,))
        lowlevel_labels_reshaped2 = expand_dims_op(lowlevel_labels_reshaped2, 1)
        loss2 = criterion(output_reshaped2, lowlevel_labels_reshaped2.long())

        lowlevel_labels3 = ms.ops.cat([lowlevel_labels[:, 0:1], lowlevel_labels[:, 2:4]], axis=1)
        output_reshaped3 = reshape_op(output3, (-1, output3.shape[-1]))
        lowlevel_labels_reshaped3 = reshape_op(lowlevel_labels3, (-1,))
        lowlevel_labels_reshaped3 = expand_dims_op(lowlevel_labels_reshaped3, 1)
        loss3 = criterion(output_reshaped3, lowlevel_labels_reshaped3.long())
    ...
    grad_fn = ms.value_and_grad(forward_fn, None, weights=optimizer.parameters, has_aux=True)
    ...
    def train_step(data, label):
        ...
        if args.clip_gradient is not None:
            grads = ops.clip_by_global_norm(grads,args.clip_gradient/3)
        optimizer(grads)
        return loss,logits
    ...
    for i, (_, _, frames, _, lowlevel_labels, _) in enumerate(train_loader):
        ...
        output  = reshape_op(output_reshaped, (32,4,106))
...
def validate(args, val_loader, model, criterion, epoch, tb_logger):
    ...
    for i, (_, _, frames, _, lowlevel_labels, _) in enumerate(val_loader):
        ...
        acc, success_rate, _ = accuracy(output_reshaped, lowlevel_labels_reshaped, max_traj_len=args.max_traj_len)
...
def save_checkpoint(state, dic, is_best, filename):
    ...
    ms.save_checkpoint(save_obj=state, append_dict=dic, ckpt_file_name=filename + '_latest.ckpt')
