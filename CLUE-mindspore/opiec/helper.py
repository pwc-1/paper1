...
def getEmbeddings(model, phr_list, embed_dims):
    ...
    already=dict()
    for phr in phr_list:
        if phr in already.keys():
            embed_list.append(already[phr])
        else:
            if phr in model.key_to_index:
                embed_list.append(model.word_vec(phr))
                already[phr]=model.word_vec(phr)
                all_num += 1
            else:
                ...
                if len(wrds) == 0:
                    ...
                    already[phr] = vec / 10000
                else:
                    embed_list.append(vec / len(wrds))
                    already[phr] = vec / len(wrds)
