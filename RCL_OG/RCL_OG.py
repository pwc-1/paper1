...
class DAG_RL:
    def __init__(self, config, data, prune_graph):
        ...
        self.q_nets = [q_net(config.num_variables, config.num_samples, config.embed_dim, config.hidden_dim, config.heads,
                           config.dropout_rate, config.device) for _ in range(config.num_variables)]
        self.optimizers = [torch.optim.Adam([
            {
                'params': self.q_nets[i].trans_encoder.parameters(), 'lr': self.config.learning_rate
            },
            {
                'params': self.q_nets[i].trans_decoder.parameters(), 'lr': self.config.learning_rate
            },
            {
                'params': self.q_nets[i].MLP.parameters(), 'lr': self.config.learning_rate
            }
    ...
    def learn(self):
        ...
        for i in tqdm(range(self.config.epoch)):
            q_values = self.q_nets[layer](torch.tensor(data_batch), torch.tensor(states))
            ...
            if self.replays[layer].length() >= self.config.batch_size:
                ...
                loss = self.cal_loss(samples, data_batch, layer)
                self.optimizers[layer].zero_grad()
                loss.backward()
                self.optimizers[layer].step()
    ...
    def cal_loss(self, samples, data_batch, layer):
        q_value = self.q_nets[layer](torch.tensor(data_batch), torch.tensor(samples['state']))
        source = torch.sum(torch.mul(q_value, torch.tensor(np.squeeze(samples['action']))), -1)
        ...
        if layer == self.config.num_variables-1:
            ...
            next_q_values = self.q_nets[layer+1](torch.tensor(data_batch), torch.tensor(samples['next_state']))
            target_rewards = rewards + np.log(np.sum(np.exp(next_q_values.detach().numpy().astype(np.float64)), -1) + 1e-8)
        targets = torch.tensor(target_rewards, dtype=torch.float32)
        loss = torch.nn.MSELoss(reduction='mean')(targets, source)
        return loss
    ...
    def get_probability(self, state, action):
        ...
        q_value = self.q_nets[layer](torch.tensor(self.get_batch(self.data.transpose(), 1)), torch.tensor(self.get_batch([state], 1)))
