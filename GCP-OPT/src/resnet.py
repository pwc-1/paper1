...
def _conv3x3(in_channel, out_channel, stride=1, use_se=False, res_base=False):
    if use_se:
        ...
        weight = mindspore.Tensor(shape=(out_channel, in_channel, 3, 3), dtype=mindspore.float32, init=One())
...
def _conv1x1(in_channel, out_channel, stride=1, use_se=False, res_base=False):
    if use_se:
        ...
        weight = mindspore.Tensor(shape=(out_channel, in_channel, 1, 1), dtype=mindspore.float32, init=One())
...
def _conv7x7(in_channel, out_channel, stride=1, use_se=False, res_base=False):
    if use_se:
        ...
        weight = mindspore.Tensor(shape=(out_channel, in_channel, 7, 7), dtype=mindspore.float32, init=One())
...
class ResNet(nn.Cell):
    ...
    def __init__(self,
        ...
        self.layer4 = self._make_layer(block,
                                       ...
                                       use_se=self.use_se,
                                       se_block=self.se_block)
        self.layer_reduce = _conv1x1(out_channels[3],128,stride = 1)
        self.layer_reduce_bn = _bn(128)
        self.Tri=Triuvec()
        self.layer_reduce_relu = nn.ReLU()
        self.fc = _fc(8256, num_classes)
