...
class LossCallBack(LossMonitor):
    ...
    def step_end(self, run_context):
        ...
        if self._per_print_times != 0 and cb_params.cur_step_num % self._per_print_times == 0:
            ...
            print("epoch: {} step: {}, loss is {}".format((cb_params.cur_epoch_num + int(self.has_trained_epoch),cur_step_in_epoch, loss)))
...
def load_pre_trained_checkpoint():
    ...
    if config.pre_trained:
        if os.path.isdir(config.pre_trained):
            ...
            if not ckpt_files:
                logger.warning("There is no ckpt file in {ckpt_save_dir}, pre_trained is unsupported.")
            else:
                ckpt_files.sort(key=os.path.getmtime, reverse=True)
                time_stamp = datetime.datetime.now()
                print("time stamp {time_stamp.strftime('%Y.%m.%d-%H:%M:%S')} pretrained ckpt model {ckpt_files[0]} loading")
            ...
            print("Invalid pre_trained {config.pre_trained} parameter.")
    return param_dict
